...
def AGV(IP, Puerto, Sensor, Handlers, goal):
    sim.simxFinish(-1)  # Cierre cualquier conexión previa
    clientID = sim.simxStart(IP, Puerto, True, True, 5000, 5)  # Conéctate a CoppeliaSim
    print(IP)
    print(Sensor)
    if clientID != -1:
    ...
    returnCode,h1 = sim.simxGetObjectHandle(clientID, Handlers[0], sim.simx_opmode_blocking)
    returnCode,h2 = sim.simxGetObjectHandle(clientID, Handlers[1], sim.simx_opmode_blocking)
    returnCode,h3 = sim.simxGetObjectHandle(clientID, Handlers[2], sim.simx_opmode_blocking)
    returnCode,h4 = sim.simxGetObjectHandle(clientID, Handlers[3], sim.simx_opmode_blocking )
    h = [h1, h2, h3, h4]
    ...
    while 1:
        ...
        ##############################################################################################################################################################################
        points = [(p1[0]+0.09, p1[1]-0.19, 1), (p2[0]-0.09, p2[1]-0.19, 1), (p3[0]-0.09, p3[1]+0.19, 1), (p4[0]+0.09, p4[1]+0.19,1)]
        ...
        x_positions, y_positions = slam.Slam_Kalman(clientID, points, Sensor[0], Sensor[1])
        ...
        #goal = (0.0,0.0) #definir objetivo
        #print(center)
        #path = dbs.move_robot(grid, center, goal, bloqueos)
        #print(path)
        while 1:
            path = dbs.move_robot(grid, center, goal, bloqueos)
            if path:
